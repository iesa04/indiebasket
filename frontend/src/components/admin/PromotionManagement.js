...
const PromotionManagement = () => {
  ...
  const handleUpdatePromotion = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`http://localhost:5000/api/promotions/${currentPromotion._id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
  ...
  const handleToggleStatus = async (id) => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/promotions/${id}/status`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
