...
export const CartProvider = ({ children, user }) => {
  ...
  const updateCartItem = async (itemId, quantity) => {
    try {
      ...
      const response = await axios.put(
        `http://localhost:5000/api/customer/cart/item/${itemId}`,
        { quantity },
        { withCredentials: true }
      );
      setCart(response.data);
  ...
  const acceptNewPrice = async (itemId) => {
    try {
      setCartUpdating(true);
      const response = await axios.put(
        `http://localhost:5000/api/customer/cart/item/${itemId}/accept-price`,
        {},
        { withCredentials: true }
      );
      setCart(response.data);
  ...
  const acceptNewStock = async (itemId) => {
    try {
      setCartUpdating(true);
      const response = await axios.put(
        `http://localhost:5000/api/customer/cart/item/${itemId}/accept-stock`,
        {},
        { withCredentials: true }
      );
      setCart(response.data);
